...
\end{figure}
\\\texttt{Kafka} è pensato per essere scalabile, fault-tollerant e ad alte prestazioni.
É scalabile in quanto è possibile aggiungere o rimuovere \texttt{Brokers} in modo dinamico, senza interrompere il servizio.
É fault-tollerant in quanto è possibile configurare repliche dei \textit{Brokers} in modo da garantire che i dati siano sempre disponibili, anche in caso di guasto di un \texttt{Broker}.
...
permettendo di distribuire sia il carico computazionale si il carico di rete, di modo che nessun \texttt{Broker} sia sovraccaricato.
...
Gli \textbf{Eventi} infatti non sono eliminati dopo esser stati letti da un \textit{Consumer}, anzi, non è nemmeno possibile eliminarli manualmente.
...
A differenza degli altri \textit{message broker} Kafka è molto improntato sulla scalabilità, per permettere ciò i  messaggi non vengono gestiti da un unico serivizio,
bensi da un insieme di servizi indipendanti detti \textbf{Brokers}. Questo gruppo di \textit{Brokers} che lavorano insieme costituisce un \textit{Kafka Cluster}.
...
I \textit{brokers} permettono anche un meccanismo di replica dei dati, sia per scopi di scalabilità che per garantire la fault-tollerance.
...
ogni partizione ha un almeno un \textit{leader} ed un numero arbitratio di \textit{followers}.\\\\
...
tutto ciò per evitare problemi di incosistenza dei dati senza bisogno di introdurre meccanismi di mutuo aggiornamento tra i \textit{brokers}. 
