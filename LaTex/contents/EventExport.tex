...
\textbf{Broadcast State} per la distribuzione della cache tra i vari nodi.

\subsection{Broadcast State}
\label{subsec:BroadcastState}
Il \textit{Broadcast State} è un pattern di \textit{Flink} che permette di distribuire uno stato tra tutti i nodi di un job.
Questo stato è distribuito in maniera efficiente e scalabile, inoltre è possibile aggiornarlo in maniera asincrona.
Infatti quando un nuovo aggioranmento dello stato viene inviato, \textit{Flink} si occupa di distribuirlo in maniera efficiente e di aggiornare lo stato su tutti i nodi.
